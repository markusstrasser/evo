...
Canonical text lives in `docs/GOALS.md`. Covers: mission, generative principle
(minimize spec surface / maximize kernel power), project mode (solid outliner
with clean extension surface), principles (kernel-purity, three-op invariant,
REPL-verifiable, specs-are-the-product, docs-facts-not-plans, test-portability,
commit-freely), autonomy boundaries (autonomous vs ask-first).
