...
- If you point `workspace` or `parse` at paths inside this repository, the tools will create `.semtools/` and `.parse/` cache directories; these are already listed in `.gitignore` so temporary embedding databases stay out of version control.
