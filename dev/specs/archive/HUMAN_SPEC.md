...
_Archived_: kept as a rich example library of before/after scenarios using the `[->]/[*]/[F]/[Z]` notation. The canonical behavioral source is now `dev/specs/LOGSEQ_SPEC.md`, which summarizes the same feel in a more compact, cross-referenced format.

The content below is the original human-oriented walkthrough (unchanged).
