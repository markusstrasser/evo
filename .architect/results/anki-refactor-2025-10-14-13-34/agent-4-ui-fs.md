...
[2025-10-14T20:35:19] thinking

**Proposing concise refactoring suggestions**

I’m outlining key areas for simplifying UI component duplication, async file handling, and event wiring. I’ll gather before/after snippets with line refs from `ui.cljs` and `fs.cljs` that propose data-driven UI maps, unified async helpers, and handler maps to replace big case statements—all aimed at reducing LOC without changing behavior.
[2025-10-14T20:35:20] codex

**Findings**

- ui.cljs:177 – `review-card` repeats near-identical scaffolding for each card type (front/back/class name lookup plus shared wrapper and buttons).  
  **Refactor:** move per-type rendering into a data map; keep the shared chrome in one place.  
  ```clojure
  ;; Before
  (defn review-card [{:keys [card show-answer?]}]
    (let [{:keys [front back class-name]}
  ...
  (def card-views
    {:qa {:class "qa-card"
          :front (fn [card _] [:div.question [:h2 "Question"] [:p (:question card)]])
          :back  (fn [card _] [:div.answer   [:h2 "Answer"]   [:p (:answer card)]])}
     :cloze {:class "cloze-card"
             :front (fn [{:keys [template deletions]} show?]
                      (let [deletion (first deletions)
                            masked   (str/replace template
                                                   (re-pattern (str "\\[" deletion "\\]"))
                                                   "[...]")]
                        [:div.cloze-text [:p (if show? template masked)]]))}
     :image-occlusion/item {:class "image-occlusion-item-card"
                            :front render-occlusion-front
                            :back  (fn [card _] [:div.answer [:p (:answer card)]])}})
  
  (defn review-card [{:keys [card show-answer?]}]
    (let [{:keys [class front back]} (get card-views (:type card) default-view)]
      [:div.review-card {:class class}
       (front card show-answer?)
       (when (and back show-answer?) (back card show-answer?))
       (if show-answer? (rating-buttons)
                        [:button {:on {:click [::show-answer]}} "Show Answer (Space)"])]))
  ```
  This trims ~35 LOC, keeps existing behaviour, and makes new card types a one-function addition.

- fs.cljs:78 – `collect-async-iterator` plus `process-md-entry` plus `load-all-md-files` form a three-step pipeline that loops manually and re-wraps promises for each branch. Using a single depth-first walker reduces ceremony.  
  ```clojure
  ;; Before
  (defn- collect-async-iterator [iterator]
    (p/loop [acc []]
      (p/let [item (.next iterator)]
        (if (.-done item)
          acc
  ...
  (defn- process-md-entry [entry path-prefix]
    (let [name (.-name entry)
          kind (.-kind entry)]
      (js/console.log "Processing entry:" name "kind:" kind)
      (cond
  ...
  ```clojure
  ;; After
  (defn- walk-md-files [path-prefix entry]
    (let [name (.-name entry)
          as-path (fn [n] (cond-> n (seq path-prefix) (str path-prefix "/")))]
      (case (.-kind entry)
        "file" (if (.endsWith name ".md")
                 (p/let [content (get-file-content entry)]
                   [{:deck (or path-prefix "default")
                     :filename name
                     :content content}])
                 (p/resolved []))
        "directory" (load-all-md-files entry (as-path name))
        (p/resolved []))))
  
  (defn load-all-md-files
    ([dir] (load-all-md-files dir ""))
    ([dir prefix]
     (-> (.values dir)
         js/Array.fromAsync                                         ; browser-supported async gather
         (p/then #(p/all (map (partial walk-md-files prefix) %)))
         (p/then #(into [] cat %)))))
  ```
  This folds three helpers into one traversal, shaving ~25 LOC and removing custom promise loops while keeping the same public API.

- ui.cljs:324 – `handle-event` is a 150+ line `case`, and keyboard shortcuts duplicate action dispatch. A registry map keeps behaviour identical but turns additions into data entries.  
  ```clojure
  ;; Before (excerpt)
  (defn handle-event [_ [action & args]]
    (case action
      ::select-folder
      (p/let [...]
        ...)
  ...
  (defn handle-keydown [e]
    (case screen
      ...
      (cond
        (and (= key " ") (not show-answer?))
        (do ... (handle-event nil [::show-answer]) ...)
        ...)))
  ```
  ```clojure
  ;; After
  (def action-handlers
    {::show-answer (fn [_ _] (swap! !state assoc :show-answer? true))
     ::select-deck (fn [_ [deck-name]]
                     (swap! !state assoc :selected-deck (when (seq deck-name) deck-name)))
     ::select-folder handle-select-folder
     ::rate-card handle-rate-card
     ::undo handle-undo
     ::redo handle-redo
     ::create-occlusion (fn [_ _] (creator/reset-creator!) (swap! !state assoc :screen :create-occlusion))
     ::save-occlusion-card handle-save-occlusion
     ::cancel-occlusion (fn [_ _] (creator/reset-creator!)
                                  (swap! !state assoc :screen :review))
     ::toggle-stats (fn [_ _] (swap! !state update :show-stats? not))})
  
  (defn handle-event [_ [action & args]]
    (if-let [f (get action-handlers action)]
      (f action args)
      (js/console.warn "Unknown action:" action)))
  ...
  (defn handle-keydown [e]
    (when-let [action (get-in key-bindings [(:screen @!state) (.-key e)])]
      (.preventDefault e)
      (handle-event nil action)
      (render!)))
  ```
  Splitting heavy branches into dedicated helpers (`handle-select-folder`, `handle-rate-card`, etc.) and reusing them for keyboard shortcuts eliminates duplicated logic and drops ~50 LOC while keeping behaviour unchanged.

**Next Steps**

1. Decide which refactors you want to adopt first (UI map, FS traversal, handler registry).  
2. Apply incrementally, rerun `npm run lint`/tests after each to ensure parity.
[2025-10-14T20:36:12] tokens used: 3,513
